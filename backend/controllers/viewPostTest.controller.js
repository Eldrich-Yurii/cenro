...
export const viewPostTest = async (req, res) => {
  try {
    ...
    if (!application || !application.postTestPath) {
      return res.status(404).json({ message: "File not found" });
    }

    const fileName = application.postTestPath;
    // const filePath = path.join(__dirname, "uploads", "assessmentCertificates", path.basename(fileName));
    const filePath = path.join(__dirname, "..", "uploads", "postTests", path.basename(fileName));

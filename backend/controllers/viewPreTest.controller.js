...
export const viewPreTest = async (req, res) => {
  try {
    ...
    if (!application || !application.preTestPath) {
      return res.status(404).json({ message: "File not found" });
    }

    const fileName = application.preTestPath;
    // const filePath = path.join(__dirname, "uploads", "assessmentCertificates", path.basename(fileName));
    const filePath = path.join(__dirname, "..", "uploads", "preTests", path.basename(fileName));
